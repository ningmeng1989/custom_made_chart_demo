...
.factory('taExecCommand', ['taSelection', 'taBrowserTag', '$document', function(taSelection, taBrowserTag, $document){
	...
	return function(taDefaultWrap, topNode){
		...
		return function(command, showUI, options, defaultTagAttributes){
			...
			try{
    //            $document[0].execCommand("styleWithCSS", null, true);
                if(_browserDetect.ie && (command == 'justifyCenter' || command == 'justifyRight' || command == 'justifyLeft')){
                    console.log(command);
                    console.log(selectedElement);
                    console.log($selected);
                    console.log($selected.css("text-align"));
                    var align = $selected.css("text-align");
            //        if(align != "center" && align != "right"){
                        $selected.css("text-align","");
            //        }
                    $selected.parents("p").css("text-align","");
                    $selected.children("p").css("text-align","");
                }
					...
					var processpaste = function(text) {
						...
						if(text && text.trim().length){
							...
							if(text.match(/class=["']*Mso(Normal|List)/i)){
                                ...
                                //移除color 和background 属性
                                text = text.replace(/color/g,'invalid');
                                text = text.replace(/background-color/g,'invalid');
