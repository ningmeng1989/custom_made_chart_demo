...
(function(exports, $) {
  ...
  var Trip = function() {
    ...
    this.settings = $.extend({
      ...
      tripBlockHTMLALL: [
        '<div class="trip-block" style="max-width:280px">',
        '<a href="#" class="trip-close"></a>',
        '<div class="trip-header"></div>',
        '<div class="trip-content" style="padding-bottom: 5px"></div>',
        '<div class="trip-progress-wrapper" style="text-align: right;margin-right: 10px;margin-bottom: 5px;">',
        '<div class="trip-progress-bar"></div>',
        '<a class="trip-url" href="#">详情</a>',
        '</div>',
        '</div>'
      ]
    }, userOptions);
...
//zj
    ...
    showCurrentTripall: function(tripindex,o) {

      this.setTripBlockall(tripindex,o);
      this.showTripBlockall(o);
    ...
    runall: function() {
      for(var i=0;i<this.tripData.length;i++){
        var tripClassId=".tripindex"+(i+1);
        var tripObject = this.tripData[i];
        var tripStart = tripObject.onTripStart || this.settings.onTripStart;
        var tripChange = tripObject.onTripChange || this.settings.onTripChange;
        var delay = tripObject.delay || this.settings.delay;
    ...
    setTripBlockall: function(tripindex,o) {
      var $tripBlock = $(tripindex).get(0);
      var that = this;
    ...
    createTripBlockall: function() {
      // make sure the element doesn't exist in the DOM tree
      if (typeof $('.trip-block').get(0) === 'undefined') {
        var that = this;
        var tripBlockHTML = this.settings.tripBlockHTMLALL.join('');
        for(var i=0;i<this.tripData.length;i++){
          var $tripBlock = $(tripBlockHTML).addClass(this.settings.tripTheme).addClass("tripindex"+i);
          $('body').append($tripBlock);
          $tripBlock.find('.trip-close').on('click', function(e) {
            e.preventDefault();
            that.cleanup();
          });
          if(this.tripData[i].url){
            var tripUrl=this.tripData[i].url;
            $tripBlock.find('.trip-url').on('click','',tripUrl, function(e) {
              e.preventDefault();
              window.open(e.data);
            });
          }
