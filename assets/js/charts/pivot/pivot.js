/**
 * Created by fuyw on 2016/6/22.
 */
'use strict';

!function (global) {
    var Pivot = global.Pivot = function (options) {
        this.options = options;
        this.pgridwidget = null;
    };

    var _INSTANCE_ID = "pivot_instance_id";
    var _instances = {};

    Pivot.prototype.render = function () {
        var self = this;
        setTimeout(function() {
            var container = self.options.dom;

            _setSize(self.options, container);

            if (!self.pgridwidget) {
                // first time
                self.pgridwidget = new Pivot.orb.pgridwidget(self.options);
                self.pgridwidget.render(container);
            } else {
                self.pgridwidget.setConfig(self.options);
            }
        });
    };

    Pivot.prototype.resize = function() {
        var size = _getSize(this.options, this.options.dom);
        this.pgridwidget.updateSize(size);
    };

    Pivot.setThemeColor = function(obj) {
        Pivot._themeColor = obj.reverse();

        //if (!this.pgridwidget) {
        //    this.options.option.series[0].background.color = obj.color;
        //} else {
        //    this.pgridwidget.setBackgroundColor(obj.color);
        //}
    };

    Pivot.prototype.dispose = function () {
        var $container = $(this.options.dom);
        var instanceId = $container.data(_INSTANCE_ID);
        $container.removeData(_INSTANCE_ID).empty();
        delete _instances[instanceId];
    };

    Pivot.prototype.on = function (event, callback) {
        // 目前只支持click事件
        if (event.type === 'click') {
            this.pgridwidget.callback = callback;
        }
    };

    Pivot.prototype.setOptions = function (options) {
        this.options = options;
    };

    Pivot.getInstance = function (options) {
        var dom = options.dom;
        var instanceId = $(dom).data(_INSTANCE_ID);
        if (!instanceId) {
            instanceId = new Date().getTime();
            var instance = new this(options);
            $(dom).data("pivot_instance_id", instanceId);
            _instances[instanceId] = instance;
        } else {
            _instances[instanceId].setOptions(options);
        }
        return _instances[instanceId];
    };

    function _setSize(options, container) {
        // TODO: 是否自适应为可选
        options.option.width = $(container).width();
        options.option.height = $(container).height();
        //if (config.grid.width === "auto") {
        //    config.width = $(container).width();
        //}
        //if (config.grid.height === "auto") {
        //    config.height = $(container).height();
        //}
    }

    function _getSize(options, container) {
        return {
    ...
    return (function e(t, n, r) {
        ...
        4: [function (_dereq_, module, exports) {
            ...
            function createfield(rootconfig, axetype, fieldconfig, defaultfieldconfig) {
                ...
                return new Field({
                    ...
                    subTotal: {
                        ...
                        expandedVisible: getpropertyvalue('expandedVisible', merged.subtotals, true)
                    },
            ...
            function SubTotalConfig(options, setdefaults) {
                ...
                var defaults = {
                    ...
                    expandedVisible: setdefaults === true ? true : undefined
                };
                options = options || {};
                ...
                this.expandedVisible = options.expandedVisible !== undefined ? options.expandedVisible : defaults.expandedVisible;
            }
            ...
            module.exports.config = function (options) {
                ...
                this.styles = {};
                this.styles.xAxis = {
                    lineStyle: config.xAxis[0].axisLine.lineStyle,
                    textStyle: config.xAxis[0].axisLabel.textStyle
                };
                this.styles.yAxis = {
                    lineStyle: config.yAxis[0].axisLine.lineStyle,
                    textStyle: config.yAxis[0].axisLabel.textStyle
                };
                this.styles.data = {
                    textStyle: config.series[0].label.normal.textStyle,
                    background: {
                        enable: config.series[0].background.enable,
                        color: Pivot._themeColor
                    }
                };
                this.styles.grid = config.grid;

                this.dataSource = data.default.slice(1) || [];
                //this.canMoveFields = config.canMoveFields !== undefined ? !!config.canMoveFields : true;
                this.canMoveFields = false;
                //this.dataHeadersLocation = config.dataHeadersLocation === 'columns' ? 'columns' : 'rows';
                this.dataHeadersLocation = 'columns';
                ...
                var fields = data.default[0].map(function (title, index) {
                    return {
                        name: index + '',
                        caption: title
                    }
                });
                this.allFields = (fields || []).map(function (fieldconfig) {
                ...
                var columns = data.properties.xAxis.bind.map(function (obj) {
                    return obj.name;
                });

                var rows = data.properties.yAxis.bind.map(function (obj) {
                    return obj.name;
                });

                data = data.properties.value.bind.map(function (obj) {
                    return obj.name;
                });

                this.rowFields = (rows || []).map(function (fieldconfig) {
                ...
                this.columnFields = (columns || []).map(function (fieldconfig) {
                    fieldconfig = ensureFieldConfig(fieldconfig);
                    return createfield(self, axe.Type.COLUMNS, fieldconfig, getfield(self.allFields, fieldconfig.name));
                });

                this.dataFields = (data || []).map(function (fieldconfig) {
                    fieldconfig = ensureFieldConfig(fieldconfig);
        ...
        8: [function (_dereq_, module, exports) {
            ...
            module.exports = function (config) {
                ...
                this.setConfig = function (options) {
                    ...
                    self.rows = new axe(self, axe.Type.ROWS);
                    self.columns = new axe(self, axe.Type.COLUMNS);
                    self.dataMatrix = {};

                    self.background = self.config.styles.data.background;

                    self.pickColor = function (value, min, max) {
                        if ($.isArray(self.background.color)) {
                            return self.multiColorGen(value, min, max);
                        } else {
                            return self.singleColorGen(min, max)(value);
                        }
                    };

                    self.singleColorGen = function (min, max) {
                        var hexGen;
                        var color = self.background.color;
                        hexGen = (function () {
                            switch (color) {
                                case "green":
                                    return function (hex) {
                    ...
                    self.multiColorGen = function (value, min, max) {
                        var maxIndex = self.background.color.length - 1,
                            index;
                        if (max === min) {
                            index = maxIndex;
                        } else {
                            index = Math.round(maxIndex * (value - min) / (max - min));
                        }

                        return self.background.color[index];
                    };
                ...
                this.setBackgroundColor = function(color) {
                    this.config.styles.data.background.color = color;
                }
                ...
                this.getData = function (field, rowdim, coldim, aggregateFunc, cellClass) {
                    var value;
                    if (rowdim && coldim) {
                        ...
                        if (self.config.styles.data.background.enable) {
                            // calc data color
                            if (rowdim.isRoot ^ coldim.isRoot) {
                                if (rowdim.isRoot) {
                                    color = self.pickColor(value, self.dataMatrix.dataValues["rowGrand"][datafieldName].min, self.dataMatrix.dataValues["rowGrand"][datafieldName].max);
                                } else {
                                    color = self.pickColor(value, self.dataMatrix.dataValues["colGrand"][datafieldName].min, self.dataMatrix.dataValues["colGrand"][datafieldName].max);
                                }
                            } else if (!rowdim.isRoot) {
                                color = this.pickColor(value, self.dataMatrix.dataValues["default"][datafieldName].min, self.dataMatrix.dataValues["default"][datafieldName].max);
                            } else {
                                // 总计的总计，直接作为最大值显示
                                color = this.pickColor(value, 0, value);
                            }
                        }
                ...
                function computeColors() {
                    ...
                    for (var datafieldIndex = 0; datafieldIndex < self.config.dataFieldsCount; datafieldIndex++) {
                        ...
                        // 是否显示小计
                        var last = self.config.subTotal.visible ? self.rows.dimensionsCount : 1;
                        for (var i = 1; i <= last; i++) {
                            _.each(self.rows.dimensionsByDepth[i], function (dimension) {
                                rowLeafs[dimension.id] = dimension.id;
                            });
                            _.each(self.columns.dimensionsByDepth[i], function (dimension) {
                                colLeafs[dimension.id] = dimension.id;
                            });
                        }

                        var max = _.max(_.map(self.dataMatrix, function (obj, key) {
                            if (key == self.rows.root.id || !rowLeafs[key]) {
                                return -Infinity;
                            }
                            return _.max(_.map(obj, function (o, k) {
                                if (k == self.columns.root.id || !colLeafs[k]) {
                                    return -Infinity;
                                }
                                return o[datafield.name] || 0;
                            }));
                        }));

                        var min = _.min(_.map(self.dataMatrix, function (obj, key) {
                            if (key == self.rows.root.id || !rowLeafs[key]) {
                                return Infinity;
                            }
                            return _.min(_.map(obj, function (o, k) {
                                if (k == self.columns.root.id || !colLeafs[k]) {
                                    return Infinity;
                                }
                                return o[datafield.name] || 0;
                        ...
                        dataValues["default"][datafield.name] = {max: max, min: 0};

                        if (self.config.grandTotal.rowsvisible) {
                            var max = _.max(_.map(self.dataMatrix, function (obj, key) {
                                if (key == self.rows.root.id) {
                                    return _.max(_.map(obj, function (o, k) {
                                        if (k == self.columns.root.id || !colLeafs[k]) {
                                            return -Infinity;
                                        }
                            ...
                            dataValues["rowGrand"][datafield.name] = {max: max, min: 0};
                        }

                        if (self.config.grandTotal.columnsvisible) {
                            var max = _.max(_.map(self.dataMatrix, function (obj, key) {
                                if (key != self.rows.root.id && rowLeafs[key]) {
                                    return _.max(_.map(obj, function (o, k) {
                                        if (k == self.columns.root.id) {
                                            return o[datafield.name] || 0;
                                        }
                                        return -Infinity;
                                    }));
                            ...
                            dataValues["colGrand"][datafield.name] = {max: max, min: 0};
                        }

                    }

                    self.dataMatrix.dataValues = dataValues;
                }
            };
        ...
        14: [function (_dereq_, module, exports) {
            ...
            module.exports.header = function (axetype, headerType, dim, parent, datafieldscount, subtotalHeader) {
                ...
                function isParentExpanded() {
                    if (self.type === HeaderType.SUB_TOTAL) {
                        ...
                        return self.dim.field.subTotal.expandedVisible || !self.expanded;
                    } else {
                            ...
                            if (self.subheaders.length > 0) {
                                for (var i = 0; i < self.subheaders.length; i++) {
                                    ...
                                    if (!subheader.dim.isLeaf) {
                                        if (isRowsAxe || subheader.visible()) {
            ...
            module.exports.dataCell = function (pgrid, isvisible, rowinfo, colinfo) {
                ...
                var cellClass = HeaderType.getCellClass(this.rowType, this.colType);
                var data = pgrid.getData(this.datafield ? this.datafield.name : null, this.rowDimension, this.columnDimension, undefined, cellClass);
                //data ? console.log(data.color) : undefined;
                CellBase.call(this, {
                    axetype: null,
        ...
        15: [function (_dereq_, module, exports) {
            ...
            module.exports = function (config) {
                ...
                this.setConfig = function (options) {
                    self.pgrid.setConfig(options);
                    buildUi();
                    pivotComponent.setProps({});
                };

                this.updateSize = function (size) {
                    self.pgrid.updateSize(size);
                    buildUi();
                    pivotComponent.setProps({});
                };

                this.setBackgroundColor = function(color) {
                    self.pgrid.setBackgroundColor(color);
                    buildUi();
                    pivotComponent.setProps({});
                };
                    ...
                    if (dataCell) {
                        ...
                        if (dataCell.rowType === uiheaders.HeaderType.GRAND_TOTAL && dataCell.colType === uiheaders.HeaderType.GRAND_TOTAL) {
                            ...
                            if (dataCell.rowType === uiheaders.HeaderType.GRAND_TOTAL) {
                                title = dataCell.columnDimension.value + '/总计 ';
                            } else if (dataCell.colType === uiheaders.HeaderType.GRAND_TOTAL) {
                                title = dataCell.rowDimension.value + '/总计 ';
                            } else {
                ...
                this.click = function (dataCell) {
                    if (this.callback) {
                        var rows = [];
                        for (var rowDim = dataCell.rowDimension; rowDim.parent != null; rowDim = rowDim.parent) {
                            rows.push(rowDim.value);
                        }

                        var columns = [];
                        for (var colDim = dataCell.columnDimension; colDim.parent != null; colDim = colDim.parent) {
                            columns.push(colDim.value);
                        }

                        this.callback({
                            rows: rows.reverse(),
                            columns: columns.reverse(),
                            value: dataCell.value || 0
                        });
                    }
                };
        ...
        18: [function (_dereq_, module, exports) {
            ...
            module.exports.PivotTable = react.createClass({
                ...
                synchronizeCompsWidths: function () {
                    ...
                    var rowButtonsContainerWidth = nodes.upperbuttonsRow ? reactUtils.getSize(nodes.upperbuttonsRow.node.children[0]).width : 0;
                    ...
                    if (nodes.upperbuttonsRow) {
                        var $upperbuttons = $(nodes.upperbuttonsRow.node.children).find('.fld-btn');
                        for (var i = 0; i < $upperbuttons.length; i++) {
                            var $button = $upperbuttons.eq(i);
                            if ($button.outerWidth(true) > nodes.rowHeadersTable.widthArray[i]) {
                                nodes.rowHeadersTable.widthArray[i] = $button.outerWidth(true);
                            } else if ($button.outerWidth(true) < nodes.rowHeadersTable.widthArray[i]) {
                                $button.css('marginRight', nodes.rowHeadersTable.widthArray[i] - $button.outerWidth(true) + parseInt($button.css('marginRight')) + 'px');
                            }
                        }
                    }
                    ...
                    if (pivotContainerHeight) {
                        ...
                        var dataCellsTableHeight = Math.ceil(Math.min(
                            ...
                            Math.max(nodes.dataCellsTable.size.height, nodes.rowHeadersTable.size.height)));
                        ...
                        $('.diagonal-container').height(nodes.colHeadersTable.size.height);
                    }
                    ...
                    // 调整行高
                    var headersHeight = nodes.rowHeadersTable.size.height,
                        dataCellsHeight = nodes.dataCellsTable.size.height;
                    var headerRows = nodes.rowHeadersTable.node.rows,
                        dataRows = nodes.dataCellsTable.node.rows;
                    if (headersHeight > dataCellsHeight) {
                        for (var i = 0; i < dataRows.length; i++) {
                            var height = headerRows[i].offsetHeight + 'px';
                        ...
                        for (var i = 0; i < headerRows.length; i++) {
                            var height = dataRows[i].offsetHeight + 'px';
                            var cells = headerRows[i].cells;
                            for (var j = 0; j < cells.length; j++) {
                                cells[j].style.height = height;
                            }
                        }
                ...
                render: function () {
                    ...
                    //if (config.styles.grid) {
                    //    tblStyle.marginTop = config.styles.grid.top + "px";
                    //    tblStyle.marginLeft = config.styles.grid.left + "px";
                    //}

                    //var nodes = self.pgrid.config.subTotal.visible ? null : [
                    ...
                    var lineStyle = self.pgrid.config.styles.xAxis.lineStyle;
                    var nodes = [
                                    ...
                                    React.createElement("tr", null,
                                        ...
                                        React.createElement("td", null,
                                            React.createElement("div", {
                                                    ...
                                                    style: {
                                                        borderColor: config.styles.yAxis.lineStyle.color,
                                                        borderWidth: config.styles.yAxis.lineStyle.width
                                                    }
                                                },
                    ...
                    if (tbl.rows) {
                        for (var rowIndex = 0; rowIndex < tbl.rows.length; rowIndex++) {
                            ...
                            for (var cellIndex = 0; cellIndex < currRow.cells.length; cellIndex++) {
                                ...
                                if (currCell.__orb && currCell.__orb._visible) {
                                    // cell width
                ...
                render: function () {
                    ...
                    cells = this.props.row.map(function (cell, index) {
                        ...
                        if (cell.visible() && layoutInfos) {
                            if (cell.dim) {
                                if ((cell.dim.isRoot && layoutInfos.topMostCells[cell.dim.depth - 1] === undefined) || (!cell.dim.isRoot && layoutInfos.topMostCells[cell.dim.depth] === undefined && (cell.dim.parent.isRoot))) {
                                    istopmost = true;
                                    layoutInfos.topMostCells[cell.dim.depth] = cell.dim;
                                }
                            } else if (!layoutInfos.topMostCells['0'] && cell.datafield) {
                                istopmost = layoutInfos.topMostCells['0'] = true;
                            }

                            if (!leftmostCellFound && layoutInfos.lastLeftMostCellVSpan === 0) {
            ...
            module.exports.PivotCell = react.createClass({
                    ...
                    switch (cell.template) {
                        ...
                        case 'cell-template-column-header':
                            ...
                            if (isWrapper || isSubtotal) {
                                ...
                                value = (cell.value || '&#160;') + (cell.type === uiheaders.HeaderType.SUB_TOTAL ? ' 小计' : '');
                            }
                            break;
                        ...
                        case 'cell-template-datavalue':
                            ...
                            cellClick = function () {
                                self.props.pivotTableComp.pgridwidget.click(cell);
                            };
                            break;
                        default:
                            break;
                ...
                getStyle: function() {
                    var style,
                        textStyle,
                        lineStyle;
                    var styles = this.props.pivotTableComp.pgrid.config.styles;
                    switch(this.props.cell.template) {
                        case 'cell-template-row-header':
                            style = styles.yAxis.textStyle;
                            lineStyle = styles.yAxis.lineStyle;
                            style.borderColor = lineStyle.color;
                            style.borderWidth = lineStyle.width;
                            break;
                        case 'cell-template-column-header':
                        case 'cell-template-dataheader':
                            style = styles.xAxis.textStyle;
                            lineStyle = styles.xAxis.lineStyle;
                            style.borderColor = lineStyle.color;
                            style.borderWidth = lineStyle.width;
                            break;
                        case 'cell-template-datavalue':
                            style = styles.data.textStyle;
            ...
            module.exports.DropTarget = react.createClass({
                ...
                render: function () {
                    ...
                    if (self.props.axetype === axe.Type.COLUMNS) {
                    ...
                    var style = self.props.axetype === axe.Type.ROWS ? {
                        ...
                        bottom: 11,
                        whiteSpace: 'nowrap'
                    } : self.props.axetype === axe.Type.COLUMNS ? {
                        position: 'absolute',
                        width: 'auto',
                        right: 0
                    } : null;
                    ...
                    return React.createElement("div", {
                        ...
                        React.createElement("table", {
                                className: self.props.axetype === axe.Type.COLUMNS ? 'inner-table' : ''
                            },
                            React.createElement("tbody", null,
                                self.props.axetype === axe.Type.COLUMNS
                                    ? [buttons]
            ...
            module.exports.PivotButton = react.createClass({
                ...
                render: function () {
                    ...
                    var gridStyle = this.props.pivotTableComp.pgrid.config.gridStyle;
                    if (gridStyle) {
                        divstyle.backgroundColor = gridStyle.header.backgroundColor;
                        divstyle.color = gridStyle.header.textColor;
                    }
                            ...
                            React.createElement("tbody", null,
                                React.createElement("tr", null,
                                    ...
                                    React.createElement("td", null, React.createElement("div", {
                                            ...
                                            className: sortDirectionClass ? ('glyphicon ' + (sortDirectionClass === 'sort-asc' ? 'glyphicon-arrow-up' : 'glyphicon-arrow-down')) : ''
                                        })
                                    ))//,
                                    //React.createElement("td", {
                                    //        className: "filter"
            ...
            module.exports.PivotTableCorner = react.createClass({
                render: function () {
                    var self = this;
            ...
            module.exports.PivotTableColumnHeaders = react.createClass({
                render: function () {
                    ...
                    var lineStyle = this.props.pivotTableComp.pgrid.config.styles.xAxis.lineStyle;
                    return React.createElement("div", {
            ...
            module.exports.PivotTableRowHeaders = react.createClass({
                ...
                render: function () {
                    ...
                    var lineStyle = this.props.pivotTableComp.pgrid.config.styles.yAxis.lineStyle;
                    return React.createElement("div", {
                            className: 'inner-table-container' + cntrClass,
                            style: {
                                position: 'absolute',
            ...
            var scrollBarMixin = {
                ...
                render: function () {
                    ...
                    var gridStyle = this.props.pivotTableComp.pgrid.config.gridStyle;
                    if (gridStyle) {
                        thumbStyle['backgroundColor'] = gridStyle.header.backgroundColor;
                        thumbStyle['borderColor'] = gridStyle.borderColor;
                    }
            ...
            var Dialog = module.exports.Dialog = react.createClass({
                ...
                componentDidMount: function () {
                    ...
                    if (styles) {
                        $(this.getDOMNode()).find('.orb-table th').css({
                            //backgroundColor: styles.xAxis.backgroundColor,
                            color: styles.xAxis.textStyle.color
                        });
                    }
