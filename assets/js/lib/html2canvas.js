...
function html2canvas(nodeList, options) {
    ...
    // modified by chartsshow: support cloneDocument property, if is not, do not clone the document
    if (!options.cloneDocument) {
        return renderDocumentCS(options, node, node.ownerDocument.defaultView.innerWidth, node.ownerDocument.defaultView.innerHeight);
    }
    else {
...
// added by chartsshow
function renderDocumentCS(options, node, windowWidth, windowHeight) {
    var oncloneHandler = (typeof(options.onclone) === "function") ? Promise.resolve(options.onclone(window.document)) : Promise.resolve(true);
    return oncloneHandler.then(function() {
        return renderWindow(node, null, options, windowWidth, windowHeight);
    });
}
...
function renderWindow(node, container, options, windowWidth, windowHeight) {
    // modified by chartsshow: container may be null, add a judgment
    if (container) {
        var clonedWindow = container.contentWindow;
    } else {
...
// modified by chartsshow: remove the 'visibility is hidden' judgment
...
function NodeParser(element, renderer, support, imageLoader, options) {
    ...
    // modified by dataviz: do not disable animations when cloneDocument is false
    if (this.options.cloneDocument) {
